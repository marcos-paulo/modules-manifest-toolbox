...
# caminho relativo (git worktree add --relative-paths): o link entre a
# raiz e o worktree passa a sobreviver se a árvore inteira for movida ou
# sincronizada pra outro caminho. Em versões mais antigas do git, cai pro
# comportamento padrão (caminho absoluto). Calculado uma vez só, na
# primeira vez que um worktree precisa ser criado.
...
function criar_worktree(){
  ...
  if ! git -C "$PATH_SCRIPT" worktree add --quiet "${GIT_WORKTREE_FLAGS_RELATIVO[@]}" "$pasta_resolvida" "$ref"; then
